"""
Mistral streaming chat provider.

Mistral provides access to various models through an OpenAI-compatible API
at https://api.mistral.ai/v1. This provider reuses the openai SDK with a
custom base_url — same approach as OpenRouter and Groq.

Get a key at https://console.mistral.ai/ and set MISTRAL_API_KEY.

Popular models for planning / code tasks (as of 2026):
  mistral-medium-latest   — Mistral's medium model
  mistral-large-latest    — Mistral's large model
  codestral-latest        — Mistral's code model

JSON mode:
  Mistral honours response_format={"type": "json_object"} for models that
  support it, which the planning pipeline relies on for structured output.

Configuration (via .env.local or environment):
  CHAT__PROVIDER=mistral
  CHAT__MODEL=mistral-medium-latest
  MISTRAL_API_KEY=...             # required
  MISTRAL_BASE_URL=https://api.mistral.ai/v1   # default; rarely changed
"""
...
# Mistral's OpenAI-compatible endpoint. Overridable via MISTRAL_BASE_URL.
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                if is_transient and attempt < self._max_retries - 1:
                    ...
                    logger.warning(
                        "mistral_chat_connection_error_retrying",
                        error=str(exc),
                        attempt=attempt + 1,
